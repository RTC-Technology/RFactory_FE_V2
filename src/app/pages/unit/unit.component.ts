...
@Component({
	...
	imports: [
		...
		HasPermissionDirective, InputNumberModule, RadioButton
	],
...
export class UnitComponent extends PermissionAwarePage implements OnInit {
	...
	readonly unitCategoryOptions = computed(() => {
		return this.categoryApi.items()
			.filter(c => c.isActive)
			.map(c => ({ label: `${c.unitCategoryCode} · ${c.unitCategoryName}`, value: c.id }));
	});
	...
	constructor() {
		...
		// console.log('unitCategoryOptions:', this.unitCategoryOptions());
	...
	readonly detailRows = signal<UnitConversionDto[]>([]);
	...
	openEdit(kind: EntityKind): void {
		...
		if (kind === 'category') {
			...
			this.form = {
				...
				unitCategoryId: u.unitCategoryId ?? 0
			};

			this.detailRows.set(
				this.conversionApi.items()
					.filter(conversion => conversion.fromUnitId === u.id)
					.map(c => ({
						...c,
						factor: c.formulaType == 1 ? c.multiplyValue : c.divideValue,
					}))
			);

			// console.log('detailRows:', this.detailRows());
		} else {
			const c = row as UnitConversionDto;
			this.form = {
				...this._emptyForm(),
				toUnitId: c.toUnitId ?? 0,
				multiplyValue: c.multiplyValue ?? 0,
				divideValue: c.divideValue ?? 0,
	...
	removeDetailRow(row: UnitConversionDto): void {
	...
	private _emptyForm(): EntityForm {
		return {
			...
			toUnitId: null, multiplyValue: 1, divideValue: 1, formulaType: 1, unitCategoryId: this.selectedCategory()?.id ?? 0,
		};
	}

	private _emptyDetailRow(): UnitConversionDto {
		return {
	...
	private _buildRequest(kind: EntityKind, id: number | null): Observable<unknown> | null {
		...
		const body = {
			unitCategoryId: this.form.unitCategoryId,
			...
			isActive: this.form.isActive,
			unitConversions: this.detailRows().map(r => ({
				id: r.id > 0 ? r.id : 0,
				fromUnitId: r.fromUnitId,
				toUnitId: r.toUnitId,
				multiplyValue: (r.formulaType ?? 1) == 1 ? r.factor : 0,
				divideValue: (r.formulaType ?? 1) == 2 ? r.factor : 0,
