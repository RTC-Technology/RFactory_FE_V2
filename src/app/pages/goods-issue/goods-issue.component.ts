...
export class GoodsIssueComponent extends PermissionAwarePage implements OnInit {
    ...
    readonly filteredSerialNoGroups = signal<SelectItemGroup[]>([]);
    ...
    readonly getSerialNoSuggestions = (
        productId: number
    ): SelectItemGroup[] => {
    ...
    onSearchSerialNo(
        ...
        const groups = this.getSerialNoSuggestions(productId);

        if (!query) {
            this.filteredSerialNoGroups.set(groups);
            return;
        }

        this.filteredSerialNoGroups.set(
            groups
                .map(group => ({
                    ...group,
    ...
    onSerialFocus(
        ...
        this.onSearchSerialNo(
            { query: '' } as AutoCompleteCompleteEvent,
            productId
        );
